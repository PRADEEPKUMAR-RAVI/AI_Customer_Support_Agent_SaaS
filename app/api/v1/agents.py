...
def _require_active_claim(ticket: Ticket, staff: StaffContext) -> None:
    """Reply/notes are only valid while the ticket is actively being worked (with_agent). Resolve
    /close never clear assignee_id, so the holder check alone would wrongly allow posting on an
    already-resolved/closed ticket — guard the state too."""
    if ticket.state != TicketState.WITH_AGENT.value:
        raise AppError(
            status_code=409,
            title="Ticket is not actively being handled",
...
async def reply(
    ...
    ticket = await _load_ticket_or_404(session, ticket_id)
    _require_active_claim(ticket, staff)
