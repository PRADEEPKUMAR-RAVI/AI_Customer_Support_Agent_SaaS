    ...
    if staff is not None and staff.is_active:
        async with with_tenant(staff.tenant_id) as session:
            ...
            await emit(
                ...
                # Stable per (staff, token_version) so an outbox retry can't double-send one reset
                # email; a genuine re-request after a completed reset bumps the version.
                dedupe_key=f"password_reset:{staff.id}:{staff.token_version}",
            )
