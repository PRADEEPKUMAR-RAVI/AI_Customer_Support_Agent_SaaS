...
Required-to-unlock steps: industry, knowledge source, customer-record dataset, and agent config
(persona or welcome message). Industry used to be set at signup (always true), but is now chosen
in onboarding step 1, so it's checked here like everything else. Inviting a teammate and adding an
allowed domain stay optional — a tenant can invite staff and whitelist embed origins later from
the console, so neither blocks reaching the dashboard.
"""
...
async def get_onboarding_status(
    ...
    completed = has_industry and has_source and has_dataset and has_config
